...
func cartt():
	if nalr :
			...
			if rev==-1:
				randy = 475-151
				carr.get_node("AnimatedSprite2D").scale.y*=-1
			...
			get_tree().get_multiplayer().rpc(get_tree().get_multiplayer().get_peers()[0],get_tree().root.get_node("Node2D/Game/Car Controller"),"carre",[randx,randy,rando,speed,truck,tank,pcenttrack,rng,rev])	
...
func carre(randx,randy,rando,speed,pcenttruck,pcenttank,pcenttrack,rng,rev):
	var carr = car.instantiate()	
	carr.position = Vector2(randx,randy)
	carr.z_index = 69
	carr.speed = speed
