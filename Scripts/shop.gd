...
func _on_button_pressed() -> void:
	if get_parent().get_parent().get_node('.').count >= costpolice:
...
func _on_button_pressed2 () -> void:
	if get_parent().get_parent().get_node('.').count >= costcone:
		get_parent().get_parent().get_node(".").tl*=0.85
		get_parent().get_parent().get_node('.').count -=costcone
		costcone*=2
		button5.text = 'BUY\ncost '+str(costcone)
		play_audio()
	pass # Replace with function body.
...
func _on_button_pressed3() -> void:
	if get_parent().get_parent().get_node('.').count >= costguy:
		get_parent().get_parent().get_node(".").auto=true
		get_parent().get_parent().get_node(".").autot*=0.9
		get_parent().get_node('TheGuyHand').visible = true
		get_parent().get_parent().get_node('.').count -=costguy
...
func _on_button_pressed4() -> void:
	if get_parent().get_parent().get_node('.').count >= costdog:
		get_parent().get_node("AnimatedSprite2D/Area2D").sp*=1.05
		get_parent().get_parent().get_node('.').count -=costdog
...
func _on_button_pressed5() -> void:
	if get_parent().get_parent().get_node('.').count >= costvip:
		get_parent().get_node("AnimatedSprite2D/Area2D").pcent*=1.5
		get_parent().get_parent().get_node('.').count -=costvip
