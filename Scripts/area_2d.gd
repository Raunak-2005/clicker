...
func _on_input_event(viewport: Node, event: InputEvent, shape_idx: int) -> void:
	if (Input.is_action_just_pressed("ui_Mouse_left_Click")):
		...
		var spawnarea = $"../../ColorRect".get_global_rect()
		var randx = randf_range(spawnarea.position.x,spawnarea.end.x)
		var randy = randf_range(spawnarea.position.y,spawnarea.end.y)
		dude.global_position = Vector2(randx,randy)
		print(randx)
		print(randy)
		add_child(dude)
		dude.walk_start()
